...
#include "IR/WORLD_IR_CODES.h"
#include "IR/IR_AH.h"
#include "IR/IR_AH_Remotes.h"
#include "IR/ir_read.cpp"
      ...
      void RFID_loop() {
        ...
        if (check_select_press()) {
          if (RFIDmenu[cursor].command == 27) {
            ...
            if (RFIDmenu[cursor].command == 2 && sdcardMounted) {  //save to file
            }
            if (RFIDmenu[cursor].command == 3 && sdcardMounted) {  //Load from file
            }
          } else {
          }
          switch (currentState) {
